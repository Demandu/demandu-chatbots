# Prompt para el auditor de producción de Demandu Platform

Este archivo **es el prompt**. Se pega tal cual como cuerpo de un agente
(`.claude/agents/auditor.md`, debajo de su frontmatter), o se manda como primer
mensaje a una sesión dedicada.

Está escrito contra la superficie real, medida el 18 de septiembre de 2026. Los
números no son decorado: son lo que convierte «revisa todo» en una lista que se
puede terminar.

---

## ⚠️ Lo que hay que entender antes de usarlo

**«Cada línea, cada coma» no lo hace un agente en una pasada.** Ningún proceso
—humano o no— encuentra todos los errores de 128 migraciones, 44 rutas y 71
funciones privilegiadas leyendo una vez. Un prompt que promete eso produce un
informe que suena completo y no lo es, que es peor que no tenerlo.

**Lo que sí se puede terminar es la COBERTURA.** La superficie está contada. Cada
elemento puede llegar a un veredicto. Cuando los siete territorios están en cero
pendientes, «de principio a fin» deja de ser una aspiración y es un hecho
comprobable.

Por eso el prompt de abajo no dice «encuentra todo». Dice: **toma un territorio,
llévalo a cero, y deja constancia de cada veredicto.**
...
**Como agente**, en `.claude/agents/auditor.md`:

```yaml
---
name: auditor
description: Audita Demando Platform territorio por territorio para salir a producción — frontera, funciones definer, dinero, aislamiento, terceros, motores y tareas. Cierra una cosa por carrera con prueba, arreglo y regla mutada.
tools: Read, Write, Edit, Grep, Glob, Bash, mcp__remote-devices__device_bash, mcp__remote-devices__device_commit_files, mcp__Supabase__execute_sql, mcp__Supabase__get_advisors, mcp__Supabase__query_logs, Projects
model: opus
---
```

…y debajo, todo lo que está entre `═══ EL PROMPT ═══` y `═══ FIN ═══`.

**Por dónde empezar**, si no hay preferencia: territorio **2** (las 71 funciones
`definer`). Es la mayor superficie de seguridad, la menos mirada, y la que ya dio
cuatro rotas de cuatro revisadas.
