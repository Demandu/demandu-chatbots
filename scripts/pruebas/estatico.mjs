...
/* ═══════════════════════════════════════════════════════════════════════════
 * LOS COMENTARIOS LOS CONTESTA LANA, NO UNA FRASE HECHA
 *
 * La respuesta pública era un texto fijo: el mismo debajo del que preguntaba
 * el precio y del que escribía «qué bonito». La IA ya entendía los privados;
 * lo que faltaba era dejarla hablar arriba. Cada regla de aquí es una forma
 * concreta de que eso se quede a medias sin que nada falle.
 * ═══════════════════════════════════════════════════════════════════════════ */
describe("La respuesta pública puede ser de la IA", () => {
  const IG = sinComentarios(fs.readFileSync(path.join(SRC, "app/api/webhooks/instagram/route.ts"), "utf8"));
  const PUB = sinComentarios(fs.readFileSync(path.join(SRC, "lib/canales/comentarioPublico.ts"), "utf8"));
  const REGLAS = sinComentarios(fs.readFileSync(path.join(SRC, "lib/canales/instagramReglas.ts"), "utf8"));
  const BARRA = sinComentarios(fs.readFileSync(path.join(SRC, "components/builder/DisparadorSocial.tsx"), "utf8"));
  const ACC = sinComentarios(fs.readFileSync(path.join(SRC, "app/(dashboard)/bots/actions.ts"), "utf8"));

  test("el webhook publica lo que decide textoPublico, no el texto fijo a pelo", () => {
    /* SE MIDE DENTRO DE `atenderComentario`. Buscar «textoPublico» en todo el
     * archivo era vacuo: la función se declara ahí mismo, así que la regla
     * pasaba aunque nadie la llamara nunca. */
    const i = IG.indexOf("async function atenderComentario");
    esperar(i > 0).verdadero("desapareció atenderComentario");
    const f = IG.slice(i, IG.indexOf("\nasync function", i + 10));

    esperar(/const publica = await textoPublico\(/.test(f)).verdadero(
      "el comentario ya no pasa por textoPublico: vuelve a salir la frase fija para todos",
    );
    esperar(/if \(publica\)/.test(f)).verdadero(
      "se publica sin comprobar que hay algo que decir: un comentario en blanco es peor que ninguno",
    );
  });

  test("el modo viaja desde la base hasta la decisión", () => {
    esperar(IG.includes("respuesta_publica_modo")).verdadero(
      "el webhook no pide la columna del modo: todos los flujos se comportarían como 'texto' " +
      "y «que conteste Lana» quedaría guardado y mudo, igual que los cuatro campos de la 0033",
    );
  });

  test("la IA del comentario público no lleva herramientas", () => {
    const i = IG.indexOf("async function textoPublico");
    esperar(i > 0).verdadero("desapareció textoPublico");
    const f = IG.slice(i, IG.indexOf("\nasync function", i + 10));

    esperar(/herramientas: \[\]/.test(f)).verdadero(
      "el comentario público puede agendar o etiquetar: quien comentó «qué bonito» acabaría con cita",
    );
    esperar(/agente:/.test(f)).falso(
      "se le pasa contexto de agente, que es justo lo que enciende las herramientas",
    );
  });

  test("el respaldo de la IA nunca se publica", () => {
    esperar(/sePuedePublicar\(/.test(IG)).verdadero(
      "se publica lo que devuelva la IA tal cual: «esa no me la sé todavía» debajo de la publicación, " +
      "a la vista de todos",
    );
    /* Y la función tiene que comparar DE VERDAD con el respaldo. Sin esta
     * comprobación, un `return true` constante pasaría la regla de arriba. */
    esperar(/igual\(t\) !== igual\(r\)/.test(PUB)).verdadero(
      "sePuedePublicar dejó de comparar con el mensaje de respaldo",
    );
  });

  test("con modo IA se contesta en público aunque no haya texto escrito", () => {
    const i = REGLAS.indexOf("export function dondeContestar");
    esperar(i > 0).verdadero("desapareció dondeContestar");
    const f = REGLAS.slice(i, REGLAS.indexOf("\n}", i));

    esperar(/modo === "ia"/.test(f)).verdadero(
      "sigue exigiendo `respuesta_publica` escrita: con «que conteste Lana» no saldría nada nunca",
    );
    esperar(/modo === "texto" && !!String\(flujo\?\.respuesta_publica/.test(f)).verdadero(
      "con modo texto ya no se exige la frase escrita: publicaría un comentario vacío",
    );
  });
  ...
  test("la pantalla ofrece elegirlo y la acción lo guarda", () => {
    ...
    esperar(/modoDeRespuestaPublica\(/.test(ACC)).verdadero(
      "el modo se guarda sin sanear y la restricción de la 0108 tumbaría el guardado entero",
    );
  });

  test("los orígenes válidos salen del catálogo", () => {
    /* Escritos a mano se quedaron sin «live»: el negocio elegía «comentario en
     * un directo», guardaba, y la fila se grababa con «dm». Sin error. */
    esperar(/new Set<string>\(ORIGENES\.map/.test(ACC)).verdadero(
      "la lista de orígenes vuelve a estar escrita a mano y se desincroniza del catálogo",
    );
  });
});
 ...
 *
 * Todo esto ya se podía configurar dentro de cada flujo. Estaba escondido:
 * configurable no es lo mismo que visible.
 * ═══════════════════════════════════════════════════════════════════════════ */
describe("La pantalla simple de respuestas automáticas", () => {
  const CH = sinComentarios(fs.readFileSync(path.join(SRC, "lib/channels.ts"), "utf8"));
  const NAV = sinComentarios(fs.readFileSync(path.join(SRC, "components/builder/BotNav.tsx"), "utf8"));
  const PAG = sinComentarios(
    fs.readFileSync(path.join(SRC, "app/(dashboard)/bots/[id]/respuestas/page.tsx"), "utf8"),
  );
  const ACT = sinComentarios(
    fs.readFileSync(path.join(SRC, "app/(dashboard)/bots/[id]/respuestas/acciones.ts"), "utf8"),
  );
  const RA = sinComentarios(fs.readFileSync(path.join(SRC, "lib/canales/respuestasAutomaticas.ts"), "utf8"));

  test("la pestaña existe donde hay comentarios y no donde no los hay", () => {
    const m = CH.match(/\{\s*key:\s*"respuestas",[^}]*\}/);
    esperar(!!m).verdadero("desapareció la pestaña de comentarios del catálogo de canales");
    esperar(/instagram/.test(m[0])).verdadero("Instagram se quedó sin la pestaña");
    esperar(/whatsapp|webchat/.test(m[0])).falso(
      "se ofrece en WhatsApp o en el widget, donde no hay comentarios que contestar: " +
      "sería una tabla que nunca puede tener una fila",
    );
  });

  test("cada sección del catálogo tiene su entrada en el menú", () => {
    /* El Record<FeatureKey, …> lo cazaría el compilador, pero aquí no hay
     * compilador. Sin esto, una sección nueva se añade al catálogo y el menú
     * revienta al pintarla — o peor, no la enseña y nadie la encuentra. */
    const faltan = [];
    for (const m of CH.matchAll(/\{\s*key:\s*"(\w+)",\s*label:/g)) {
      if (!new RegExp(`\\b${m[1]}:\\s*\\{\\s*suffix:`).test(NAV)) faltan.push(m[1]);
    }
    esperar(faltan).igual([], "secciones del catálogo que el menú del chatbot no sabe pintar");
  });

  test("los sitios salen del catálogo, no de una lista a mano", () => {
    esperar(/ORIGENES\.filter\(\(o\) => o\.canales\.includes\(canal\)\)/.test(RA)).verdadero(
      "la lista de sitios vuelve a estar escrita a mano: el día que se añada uno " +
      "—los directos ya pasaron por esto— no tendría interruptor",
    );
    esperar(/superficiesDe\(/.test(PAG) && /superficiesDe\(/.test(ACT)).verdadero(
      "la pantalla y el guardado dejaron de mirar la misma lista de sitios: " +
      "se enseñaría un interruptor que al guardar no existe",
    );
  });

  test("encender un sitio crea algo que de verdad contesta", () => {
    /* Un flujo con el lienzo vacío no manda nada: el interruptor quedaría
     * encendido y el chatbot mudo. Es el fallo de esta plataforma que más
     * veces ha aparecido: se guarda y no hace nada. */
    esperar(/graph: grafoDeLana\(\)/.test(ACT)).verdadero(
      "el flujo que se crea al encender un sitio ya no lleva el bloque de Lana",
    );
    esperar(/type: "ai"/.test(RA)).verdadero(
      "grafoDeLana dejó de poner un bloque de IA: el interruptor no contestaría nada",
    );
  });

  test("apagar un sitio NO borra lo que el negocio hubiera cambiado", () => {
    const i = ACT.indexOf("export async function guardarDondeContesta");
    const f = ACT.slice(i, ACT.indexOf("\nexport async function", i + 10));
    esperar(/enabled: encendido/.test(f)).verdadero("apagar dejó de ser apagar");
    esperar(/\.delete\(\)/.test(f)).falso(
      "apagar borra el flujo: quien lo vuelva a encender el jueves perderá lo que editó el martes",
    );
  });

  test("el nombre con el que se borra una promoción NO viene del formulario", () => {
    /* Si llegara del navegador, alguien podría mandar «Lana · Mensajes
     * directos» y borrar con ello la respuesta general del chatbot. */
    esperar(/function nombrePromoSeguro/.test(ACT)).verdadero("desapareció el nombre calculado en el servidor");
    esperar(/\.eq\("name", nombrePromoSeguro\(palabra\)\)/.test(ACT)).verdadero(
      "el borrado de una promoción usa un nombre que no calcula el servidor",
    );
    esperar(/formData\.get\("nombre"\)/.test(ACT)).falso(
      "el nombre a borrar vuelve a llegar del formulario",
    );
  });
  ...
  test("una promoción en un privado no promete comentario público", () => {
    esperar(/origen === "dm" \? "no" : publica/.test(ACT)).verdadero(
      ...
      "y la pantalla enseñaría algo que no puede pasar",
    );
  });
});
