...
export type FlujoConDisparador = {
  ...
  /** `ia` · `texto` · `no`. Sin valor se comporta como `texto` (ver 0108). */
  respuesta_publica_modo?: string | null;
...
export function dondeContestar(
  ...
  /* ── QUÉ CUENTA COMO «HAY ALGO QUE DECIR» DEPENDE DEL MODO ───────────────
   *
   * Con `texto` sigue siendo lo de siempre: sin frase escrita no se publica
   * nada. Con `ia` la frase no existe todavía —la escribe Lana leyendo el
   * comentario— así que exigirla habría dejado el modo nuevo mudo para todos:
   * el negocio elegiría «que conteste Lana», guardaría, y no saldría nada.
   * Es el mismo fallo que tuvieron los cuatro campos de la 0033.
   */
  const modo = modoDeRespuestaPublica(flujo);
  const hayQue = modo === "ia" || (modo === "texto" && !!String(flujo?.respuesta_publica ?? "").trim());
