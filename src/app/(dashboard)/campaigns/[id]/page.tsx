...
function label(status: string) {
  switch (status) {
    ...
    case "read": return { t: "Leído (sin responder)", c: "bg-success/15 text-success" };
    case "delivered": return { t: "Entregado (no leído)", c: "bg-sky-500/15 text-sky-300" };
    case "sent": return { t: "Enviado", c: "bg-surface-raised text-muted" };
    case "failed": return { t: "Falló", c: "bg-danger/15 text-danger" };
    default: return { t: "En cola", c: "bg-surface-raised text-muted-2" };
  }
}
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-auto p-8">
        <Link href="/campaigns" className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted hover:text-white">
          <ArrowLeft className="h-4 w-4" /> Volver a Campañas
        </Link>

        <h2 className="font-display text-2xl font-bold text-white">{(campaign as any).name}</h2>
        <p className="mt-1 text-sm text-muted-2">
          Plantilla: <b className="text-muted">{(campaign as any).template_name ?? "—"}</b> ·{" "}
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { k: "Enviados", v: funnel.sent, c: "text-white" },
            { k: "Entregados", v: funnel.delivered, c: "text-sky-300", p: pctOf(funnel.delivered) },
            { k: "Leídos", v: funnel.read, c: "text-success", p: pctOf(funnel.read) },
            { k: "Respondieron", v: funnel.replied, c: "text-pink", p: pctOf(funnel.replied) },
          ].map((m) => (
            <div key={m.k} className="card p-4 text-center">
              <div className={`text-2xl font-bold ${m.c}`}>{m.v}</div>
              <div className="mt-0.5 text-[11px] uppercase tracking-wide text-muted-2">{m.k}{m.p ? ` · ${m.p}` : ""}</div>
            </div>
          ))}
        </div>
        ...
        <h3 className="mb-3 mt-8 font-display text-lg font-semibold text-white">Detalle por contacto</h3>
                ...
                return (
                  <tr key={r.id} className="border-t border-surface-border">
                    <td className="px-4 py-3">
                      <div className="font-medium text-white">{r.name || r.phone}</div>
                    ...
                    <td className="px-4 py-3 text-xs text-muted">{fmt(r.delivered_at)}</td>
                    <td className="px-4 py-3 text-xs text-muted">{fmt(r.read_at)}</td>
                    <td className="px-4 py-3 text-xs text-muted">{fmt(r.replied_at)}</td>
                  </tr>
                );
              })}
              {!sorted.length && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-muted-2">Sin destinatarios.</td></tr>
              )}
            </tbody>
          </table>
        </div>
