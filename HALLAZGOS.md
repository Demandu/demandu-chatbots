...
### H-01 · Tres pagos de Stripe se descartaron con un 200 mudo
**Origen:** auditoría 8 sep, punto 1.9 · `src/app/api/stripe/webhook/route.ts:72,173`

`if (!orgId) break;` sin registro y sin marcar fallo. El evento se guarda como
procesado correctamente y se contesta 200, así que Stripe no reintenta.

**Comprobado:** 3 eventos huérfanos, todos de la misma factura
`in_1UCTd6CtyvDLMclC9dlu42sk` ($64.00, precio `price_1T7letCtyvDLMclCc1ugH6T7`),
del 9 de septiembre. Ese precio **no es** ninguno de los cuatro planes actuales.

**Falta saber:** si esa factura es de un cliente real o de una prueba. Hasta
saberlo no se puede decir si alguien pagó y se quedó sin plan.

```sql
select count(*) from billing_events
where procesado_at is not null and error is null and org_id is null;
```
